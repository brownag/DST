...
All HTML, CSS, and JS in one file. Alpine.js reactive state drives the UI.
...
Generated by Python pipeline — do not edit manually.
```json
{
  "version": "1.0.0",
  ...
  "navigation": { "criteria": [...] },  // decision nodes
...
assets/*.json (USDA source)
  ...
  → validate_schema.py    Validates output structure
  → data/dst-data.json
```
